...
class LogisticRegression:
    ...
    def fit(self, input_features, target_labels):
        ...
        for epoch in range(self.epochs):
            Z = np.dot(input_features, self.weight)  # I have already added bias to the feature here: input_features = np.hstack((bias, input_features))
            P = self.softmax(Z)
    ...
    def predict(self, input_features):
        ...
        Z = np.dot(input_features_bias, self.weight)  # Adjusted for the bias term
        P = self.softmax(Z)
